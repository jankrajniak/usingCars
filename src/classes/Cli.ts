    ...
    inquirer
      .prompt([
        ...
        },
        {
          type: 'input',
          name: 'range',
          message: 'Enter the Estimated Range',
        },
        {
          type: 'input',
  ...
  async performActions(): Promise<void> {
    const answers = await inquirer
      ...
      // TODO: add statements to perform the tow action only if the selected vehicle is a truck. Call the findVehicleToTow method to find a vehicle to tow and pass the selected truck as an argument. After calling the findVehicleToTow method, you will need to return to avoid instantly calling the performActions method again since findVehicleToTow is asynchronous.
          ...
          const chosenVehicle = this.vehicles.find(vehicle => vehicle.vin === this.selectedVehicleVin);
          if (chosenVehicle instanceof Truck) {
            await this.findVehicleToTow(chosenVehicle)
          } else {
            console.log('Only trucks can tow');
          }
      } else if (answers.action === 'Pop a wheelie') {
          const chosenVehicle = this.vehicles.find(vehicle => vehicle.vin === this.selectedVehicleVin);
          if (chosenVehicle instanceof Motorbike) {
            chosenVehicle.wheelie();
          } else {
